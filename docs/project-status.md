...
| 2026-04-24 02:35 UTC | **cheatsheet 重排 + 导出 docx**:把 §3 「阶段 × 管线 × 代号」二维表重命名为「**管线 × 阶段 × 代号**」并提到 §1(层级关系总览)之后,旧 §2「代号总表」顺延为 §3;§3 子节随之重编号 §3.1–§3.13;`docs/methodology.md` 与本表的章节引用同步更新(原 §2.4 → §3.4);`pandoc` 转出 `docs/codenames-cheatsheet.docx`(28KB,14 张表全保留,中文渲染就绪) |
