...
| **⑪** | **Cross-document synthesis(C17,5%)** | ✅ v0.2 锁定 | +$2-3k(frontier 生成)|
| **⑫** | **Multi-turn refinement(C18,frontier 30%)** | ✅ v0.2 锁定 | +30% frontier API 成本 ≈ +$8k |
| **⑬** | **DSPy prompt 优化(C19,W1 一次性)** | ✅ v0.2 锁定 | $200(扫 4 个 prompt 各 100 样本)|
| **⑭** | **Retrieval-augmented grounding(C20,20% 路径)** | ✅ v0.2 锁定 | input token +30%(成本 +5%)|
...
| **A12** | **Cross-document synthesis** | 关(单 seed)| 5% 路径走多 seed 融合(C17) | C17 | **是**(2B uniq 重生成)| +0.3-0.5pp 知识连贯 | 🆕 v0.2 |
| **A13** | **Multi-turn refinement** | 单 pass | frontier 30% 走 outline→draft→refine(C18) | C18 | **是**(frontier 部分重生成)| +0.2-0.4pp | 🆕 v0.2 |
| **A14** | **DSPy 自动 prompt 优化** | 手写 prompt | DSPy 扫出的优化版 prompt(C19) | C19 | **是**(全 rephrase 重生成)| +0.1-0.3pp | 🆕 v0.2 |
| **A15** | **RAG 多 doc grounding** | 单 seed | 2-3 同主题 doc 拼成上下文(C20) | C20 | **是**(20% 路径重生成)| +0.2-0.4pp | 🆕 v0.2 |
